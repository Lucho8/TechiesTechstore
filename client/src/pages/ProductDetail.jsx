    ...
    const fetchData = async () => {
      try {
        const [productRes, relatedRes] = await Promise.all([
          ...
          fetch(`${API_URL}/api/products/related/${slug}`),
        ]);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setProduct({
          ...product,
          reviews: [reviewConUsuario, ...product.reviews],
        });
      } else {
      ...
      <div className="bg-slate-800 rounded-3xl border border-slate-700 shadow-2xl overflow-hidden flex flex-col md:flex-row mb-12">
        ...
        <div className="md:w-1/2 p-8 sm:p-12 flex flex-col justify-center">
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl font-bold text-lg transition-all duration-300 flex items-center justify-center gap-3 shadow-xl ${isAdded ? "bg-emerald-600 text-white shadow-emerald-600/30 scale-95" : product.stock > 0 ? "bg-violet-600 hover:bg-violet-500 text-white shadow-violet-600/30 hover:-translate-y-1" : "bg-slate-700 text-slate-500 cursor-not-allowed"}`}
          >
            {isAdded
          ...
          <div>
            {user ? (
              ...
              <div className="bg-slate-900 p-8 rounded-2xl border border-slate-700 text-center">
                ...
                <Link
                  to="/login"
                  className="inline-block bg-violet-600 hover:bg-violet-500 text-white font-bold py-2 px-6 rounded-lg transition-colors"
                >
                  Iniciar Sesión
                </Link>
              </div>
