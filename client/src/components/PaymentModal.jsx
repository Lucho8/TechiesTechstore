...
function PaymentModal({ total, onSuccess, onClose }) {
  ...
  const handlePayment = (e) => {
    ...
    // Simulamos la conexión con el Banco / MercadoPago (tarda 2.5 segundos)
    setTimeout(() => {
      setIsProcessing(false);
      onSuccess(); // Le avisamos al Checkout que el pago fue exitoso ✅
    }, 2500);
  };
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 w-full max-w-md overflow-hidden animate-fade-in-up">
        ...
        <form onSubmit={handlePayment} className="p-6">
          ...
          {/* Botón de Pagar con efecto de carga */}
          <button
            type="submit"
